...
def gpt_short_verbalizer(files_dir, llm_api, llm_strong, llm_base, logging):
    ...
    human_message = (
        "you are student who wrote this paper. "
    ...
    prompt = (
        "for each bullet, show specifics. if improved something, "
    ...
    prompt = (
        "convert text denoted by double angle brackets to a cohesive and educational text of up to 300 words"
    ...
    prompt = (
        "create a set of slides each with title and 3-4 bullet points, reflecting main idea in this text. "
...
def gpt_qa_verbalizer(files_dir, llm_api, llm_base, matcher, logging):
    ...
    for Q in Qs["questions"]:
        ...
        G = answer.split(".")
        seq = matcher.match(
            G[:-1], T[1:], minilm=1, bert=1, fuzz=1, spacy=1, diff=1, tfidf=1, pnt=True
        )
    ...
    for i_s, sec in enumerate(sections):
        ...
        if manual:
            ...
            num_input_tokens = len(
                encoding.encode(sys_message_func + " " + human_message)
            )
            ...
            for i in range(3):
                try:
                    response = llm_api(
                        model=llm_strong, messages=messages, temperature=0
                    )
                    gpttext = response.choices[0].message.content
        ...
        gptpagemap_section, textpagemap_section = map_gpttext_to_text(
            addedtext, curr_upd, page_inds, matcher
        )
...
def gpt_text_verbalizer(text, llm_api, llm_base, manual, include_summary, logging):
    ...
    for i_s, s in enumerate(sections):
        ...
        sys_message_func = (
            "You are an ArXiv paper audio paraphraser. Your primary goal is to "
            "rephrase the original paper content while preserving its overall "
        ...
        if include_summary:
            sys_message = (
                "As an AI specializing in summarizing ArXiv paper sections, your main task is to distill "
...
def create_summary(abstract, title, paper_id, llm_api, llm_base, files_dir):
    sys_message = (
        "Given the abstract for the paper, summarize it in 30 words or less. "
