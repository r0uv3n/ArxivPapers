...
def main(args):
    ...
    if args.create_video:
        ...
        pagemap = map_text_to_pdfpages(
            text, f"{os.path.join(files_dir, main_file)}.pdf", matcher
        )
        ...
        coords, pageblockmap = map_page_to_blocks(
            ...
            args.gs,
            files_dir,
            f"{os.path.join(files_dir, main_file)}.pdf",
            matcher,
            display,
        )
    ...
    if args.create_short:
        gpttext_short, slides_short = gpt_short_verbalizer(
            files_dir, llm_api, args.llm_strong, args.llm_base, logging
        )
        ...
        tmpdata = {
            "gpttext_short": gpttext_short,
            "gptslides_short": slides_short["slides"],
        }

    if args.create_qa:
        questions, answers, qa_pages = gpt_qa_verbalizer(
            files_dir, llm_api, args.llm_base, matcher, logging
        )
    ...
    if args.create_short:
        with open(
            os.path.join(files_dir, args.chunk_mp3_file_list), "w"
        ) as mp3_list_file:
            text_to_speech_short(
                gpttext_short,
                slides_short,
        ...
        final_audio_short = os.path.join(
    ...
    if args.create_audio_simple:
        with open(
            ...
            text_to_speech(
                gpttext, mp3_list_file, files_dir, tts_client, args.voice, logging
            )
    ...
    if main_file != renamed_main:
        ...
        shutil.copy(
            f"{os.path.join(files_dir, main_file)}.pdf",
            f"{os.path.join(files_dir, temp_filename)}.pdf",
        )
        shutil.copy(
            f"{os.path.join(files_dir, temp_filename)}.pdf",
            f"{os.path.join(files_dir, renamed_main)}.pdf",
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--paperid", type=str, default="", help="ArXiv paper id, e.g., 1706.03762"
    )
    parser.add_argument(
        "--l2h",
        action="store_true",
        help="use latex2html instead of latexmlc to convert latex to HTML",
    )
    parser.add_argument(
        "--verbose",
        type=str,
        ...
        default="info",
        help="print debugging information: debug-all, info-some",
    )
    parser.add_argument(
        "--pdflatex", type=str, default="pdflatex", help="pdflatex command"
    )
    parser.add_argument(
        "--latex2html", type=str, default="latex2html", help="latex2html command"
    )
    parser.add_argument(
        "--latexmlc", type=str, default="latexmlc", help="latexmlc command"
    )
    parser.add_argument(
        "--stop_word",
        type=str,
        default="",
        help="word/phrase in section to stop HTML parsing at",
    )
    ...
    parser.add_argument(
        "--cache_dir", type=str, default="cache", help="cache directory for BERT model"
    )
    parser.add_argument(
        "--gdrive_id",
        type=str,
        default="",
        help="ID of the Google folder where to save audio files",
    )
    parser.add_argument(
        "--voice",
        type=str,
        default="Polyglot-1",
        help="voice name for Google text-to-speech model",
    )
    parser.add_argument(
        "--final_audio_file", type=str, default="final_audio", help="final audio name"
    )
    parser.add_argument(
        "--chunk_mp3_file_list",
        type=str,
        default="mp3_list.txt",
        help="where to save chunk mp3 files",
    )
    parser.add_argument(
        "--manual_gpt", action="store_true", help="manual entry of GPT outputs"
    )
    parser.add_argument(
        "--include_summary",
        action="store_true",
        help="include summary after each section",
    )
    parser.add_argument(
        "--extract_text_only",
        action="store_true",
        help="extract only the text from paper and exit",
    )
    parser.add_argument("--create_video", action="store_true", help="create long video")
    parser.add_argument(
        "--create_short", action="store_true", help="create short video"
    )
    parser.add_argument("--create_qa", action="store_true", help="create qa video")
    parser.add_argument(
        "--create_audio_simple", action="store_true", help="create audio"
    )
    parser.add_argument(
        "--openai_key", type=str, default="", help="openai key to call GPT API"
    )
    parser.add_argument(
        "--llm_strong",
        type=str,
        default="gpt-4-0125-preview",
        help="llm model for complex tasks",
    )
    parser.add_argument(
        "--llm_base",
        type=str,
        default="gpt-3.5-turbo-0125",
        help="llm model for basic tasks",
    )

    args = parser.parse_args()

    assert not (
        args.create_audio_simple and args.create_video
    ), "Cannot create long video and simple audio at once"
