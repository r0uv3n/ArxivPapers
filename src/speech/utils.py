...
def text_to_speech_qa(
    questions, answers, mp3_list_file, files_dir, tts_client, ffmpeg, logging
):
    ...
    for q, a in zip(questions, answers):
        ...
        # append 2 seconds
        chunk_audio_with_silence = os.path.join(
...
def synthesize_speech(text, tts_client, voice, rate=1.0):
    voice_params = texttospeech.VoiceSelectionParams(
        language_code="en-US", name=f"en-US-{voice}"
    )
...
def text_to_speechvideo(
    text, mp3_list_file, files_dir, tts_client, pageblockmap, voice, logging
):
    splits = sent_tokenize(text)

    assert len(pageblockmap) == len(
        splits
    ), "Number of pageblockmap does not match number of splits"
...
def synthesize(
    ...
    if response:
        ...
        synthesize(
            chunks[0], tts_client, files_dir, mp3_list_file, page, block, voice, logging
        )
        synthesize(
            chunks[1], tts_client, files_dir, mp3_list_file, page, block, voice, logging
        )
