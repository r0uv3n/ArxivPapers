...
def crop_pdf(
    ...
    with open(input_pdf_path, "rb") as file_handle:
        ...
        page.mediabox.upper_right = (
            float(mediabox[2]) - float(to_crop_right),
            float(mediabox[3]) - float(to_crop_upper_top),
        )
        ...
        page.mediabox.lower_left = (
            float(to_crop_left),
            float(mediabox[3]) - float(to_crop_top),
        )
...
def zip_files(
    ...
    if create_short:
        files_to_zip = [
            f"{final_audio_file}_short.mp3",
            f"shorts_{chunk_mp3_file_list}",
        ]
        ...
        if os.path.exists(os.path.join(files_dir, "slides")):
            ...
            with open(
                os.path.join(files_dir, f"shorts_{chunk_mp3_file_list}"), "r"
            ) as f:
                lines = f.readlines()
        ...
        if not create_video:
            ...
            os.system(
                f"{gs} -sDEVICE=pdfwrite -dNOPAUSE -dBATCH -dFirstPage={page_num + 1} "
                f"-dLastPage={page_num + 1} -sOutputFile={os.path.join(files_dir, str(page_num))}.pdf "
                f'{os.path.join(files_dir, "main.pdf")} {display}'
            )
            # convert to PNG
            os.system(
                f"{gs} -sDEVICE=png16m -r300 -o {os.path.join(files_dir, str(page_num))}.png "
                f"{os.path.join(files_dir, str(page_num))}.pdf {display}"
            )
            os.system(
                f"{ffmpeg} -loop 1 -i {os.path.join(files_dir, str(page_num))}.png "
    ...
    if create_qa:
        files_to_zip.extend(
            [f"{final_audio_file}_qa.mp3", f"qa_{chunk_mp3_file_list}", "qa_pages.pkl"]
        )
    ...
    if create_video:
        ...
        files_to_zip.extend(
            ["main.pdf", chunk_mp3_file_list, "block_coords.pkl", "gptpagemap.pkl"]
        )
